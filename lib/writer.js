...
module.exports = {
    ...
    callExpression: function(node) {
        ...
        if (node.arguments.length) {
            args = this.middle(node.callee.end, node.arguments, afterCallee) +
                this.processArray(node.arguments, [',', ' ']) +
                this.right(node.arguments, node.end, ')');
        } else {
            afterCallee.push(')');
            args = this.right(node.callee.end, node.end, afterCallee);
        }
        return callee + args;
    },
    ...
    processNode: function(node) {
        ...
        try {
            switch (node.type) {
                ...
                case 'BlockStatement':
                    if (node.body.length) {
                        return this.right(node.start, node.body, '{') +
                            this.processArray(node.body, ['\n']) +
                            this.left(node.body, node.end, '}');
                    } else {
                        return this.middle(node.start, node.end, ['{', '}']);
                    }
                    break;
                ...
                case 'ThrowStatement':
                    return this.right(node.start, node.argument.start, ['throw', ' ']) +
                        this.processNode(node.argument) +
                        this.right(node.argument.end, node.end, ';');

                case 'ReturnStatement':
                    if (node.argument) {
                    return this.right(node.start, node.argument.start, ['return', ' ']) +
                        this.processNode(node.argument) +
                        this.right(node.argument.end, node.end, ';');
                    } else {
                ...
                case 'CatchClause':
                    return this.right(node.start, node.param.start, ['catch', ' ', '(']) +
                ...
                case 'ExportNamedDeclaration':
                    return 'export ' + this.processNode(node.declaration);
                ...
                default:
                    console.warn('Don\'t know how to write type "' + node.type + '"');
    ...
    _iterateBetween: function (start, end, tIndex, cIndex, whitelist) {
         var current = start,
            str = '',
            wht = whitelist && whitelist.filter(function(item) {
                 return /\S/.test(item);
             });
        while(current < end) {
            ...
            if (token.start === current && (!wht || wht[0] === token.value)) {
                if (wht) {
                    wht.shift();
                }
                str += token.value;
