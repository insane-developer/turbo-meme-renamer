...
module.exports = {
    ...
    wordWithLabelStatement: function(name, node) {
        var label = '';
        if (node.label) {
            label = this.csp(node.start + name.length, node.label.start, ' ') +
    ...
    binaryExpression: function (node) {
        ...
        if (this.opPrecedence(node.left) < curOp) {
            left = this.csp(node.start, node.left.start, '(') +
                left +
                this.csp(node.left.end, node.right.start, ')');
        }

        middle = this.csp(node.left.end, node.right.start, [' ', node.operator, ' ']);

        if (this.opPrecedence(node.right) < curOp) {
            right = this.csp(node.left.end, node.right.start, '(') +
                right +
                this.csp(node.right.end, node.end, ')');
        }
    ...
    callExpression: function(node) {
        ...
        if (node.arguments.length) {
            args = this.csp(node.callee.end, node.arguments, '(') +
                this.processArray(node.arguments, [',', ' ']) +
                this.csp(node.arguments, node.end, ')');
        } else {
            args = this.csp(node.callee.end, node.end, ['(', ')']);
        }
        return callee + args;
    },
            ...
            switch (node.type) {
                ...
                case 'SwitchCase':
                    ...
                    if (node.consequent.length) {
                        ...
                        return prepend + this.csp(node.test ? node.test.end : node.start + 7, node.end, ':');
                    }
                    break;
                ...
                case 'ReturnStatement':
                    ...
                    return this.csp(node.start, node.argument.start, ['return', ' ']) +
                        ...
                        return this.csp(node.start, node.end, ['return', ';']);
                    }
                ...
                case 'AssignmentExpression':
                    return this.processNode(node.left) +
                        this.csp(node.left.end, node.right.start, [' ', node.operator, ' ']) +
                        this.processNode(node.right);
                ...
                case 'MemberExpression':
                    ...
                    if (objPrecedence < nodePrecedence ||
                        ...
                        str = this.csp(node.start, node.object.start, '(') +
                        this.processNode(node.object) +
                        this.csp(node.object.end, node.property.start, ')');
                    } else {
                ...
                case 'UnaryExpression':
                    ...
                    return node.operator +
                        ...
                        this.processNode(node.argument) +
                        this.csp(node.argument.start, node.end);
